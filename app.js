...
function copyText(text) {
  if (navigator.clipboard && window.isSecureContext) {
    ...
    ta.focus();
    ta.select();
    try { document.execCommand("copy"); toast("کپی شد ✓"); }
...
function renderRoutePage(slug) {
  // چک redirect برای hash-based links
  var hashRedirects = Object.assign({}, redirects, getSavedRedirects());
  var hashPath = "/" + siteConfig.routeWord + "/" + slug;
  if (hashRedirects[hashPath]) {
    window.location.replace(hashRedirects[hashPath]);
    return;
  }
...
function showCode(slug) {
  ...
  box.innerHTML =
    ...
    '<pre class="code-pre" id="page-code-box">' + code + '</pre>' +
    '<button class="btn btn-primary" onclick="copyById(\'page-code-box\')">کپی کد</button>';
...
function showRedirectCode(from, to) {
  ...
  box.innerHTML =
    ...
    '<div class="code-step">' +
      '<span class="code-step-label">① فایل <code>pages.js</code> — داخل آبجکت <code>redirects</code></span>' +
      '<pre class="code-pre" id="rc-pages">' + pagesCode + '</pre>' +
      '<button class="btn btn-primary" onclick="copyById(\'rc-pages\',\'کد pages.js کپی شد ✓\')">کپی — برای pages.js</button>' +
    '</div>' +
    '<div class="code-step" style="margin-top:16px">' +
      '<span class="code-step-label">② فایل <code>index.html</code> — داخل آبجکت <code>var defaults</code></span>' +
      '<pre class="code-pre" id="rc-index">' + inlineCode + '</pre>' +
      '<button class="btn btn-primary" onclick="copyById(\'rc-index\',\'کد index.html کپی شد ✓\')">کپی — برای index.html</button>' +
    '</div>';
  box.scrollIntoView({ behavior: "smooth" });
}
...
function copyById(id, msg) {
  ...
  if (msg) setTimeout(function(){ toast(msg); }, 50);
}
